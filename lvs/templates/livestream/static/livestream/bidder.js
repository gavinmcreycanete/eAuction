...
if(OT.checkSystemRequirements() == 1){ // Check if this browser supports WebRTC.
	...
	session.on("streamCreated", function(event) { // Check if the stream has created in a certain session.
		...
		xhttprequest.open("GET", "/lvs/start_archive/", true); // Request for start recording the video.
		xhttprequest.send();	
	});
	...
	session.on("streamDestroyed", function(event) {	
		...
		xhttprequest.open("GET", "/lvs/end_archive/", true); // Request for stop recording the video.
		xhttprequest.send();
